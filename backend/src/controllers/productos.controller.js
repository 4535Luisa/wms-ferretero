...
const buscarProducto = async (req, res) => {
  ...
  const { data, error } = await supabase
    .from("productos")
    .select(
      "id, codigo_interno, descripcion_corta, unidad_empaque, codigo_barras, familia",
    )
    .or(
      `codigo_interno.ilike.${referencia.trim()},codigo_barras.eq.${referencia.trim()}`,
    )
    .eq("activo", true)
    .single();
...
const inventarioGeneral = async (req, res) => {
  // Query directa — resuelve ubicaciones en consultas separadas
