...
const buscarProducto = async (req, res) => {
  ...
  const { data, error } = await supabase
    .from("productos")
    .select("id, codigo_interno, descripcion_corta, unidad_empaque, codigo_barras, familia")
    .or(`codigo_interno.ilike.${referencia.trim()},codigo_barras.eq.${referencia.trim()}`)
    .eq("activo", true)
    .single();
  if (error || !data)
...
const buscarPorBarras = async (req, res) => {
  ...
  const { data, error } = await supabase
    .from("productos")
    .select("id, codigo_interno, descripcion_corta, unidad_empaque, codigo_barras")
...
const historialProducto = async (req, res) => {
  ...
  const [invRes, bitacoraRes] = await Promise.all([
    supabase
      .from("inventario")
      .select("id, cantidad_disponible, cantidad_comprometida, ubicacion_id, bodega_id, ubicaciones(codigo), bodegas(codigo, nombre)")
      .eq("producto_id", id),
    supabase
      .from("bitacora")
      .select("id, accion, valores_antes, valores_despues, created_at, usuarios(nombre)")
      .eq("tabla", "inventario")
...
const inventarioGeneral = async (req, res) => {
  const { data: rows, error } = await supabase
    .from("inventario")
    .select("producto_id, cantidad_disponible, cantidad_comprometida, ubicacion_id, bodega_id")
    .order("producto_id");
  if (error) return sendServerError(res, error, req);
  const productoIds = [...new Set((rows || []).map((r) => r.producto_id))];
  const bodegaIds = [...new Set((rows || []).map((r) => r.bodega_id))];
  const ubicacionIds = [...new Set((rows || []).map((r) => r.ubicacion_id).filter(Boolean))];
  const [prodsRes, bodsRes, ubicsRes] = await Promise.all([
    supabase.from("productos").select("id, codigo_interno, descripcion_corta").in("id", productoIds),
  ...
  const prodMap = Object.fromEntries((prodsRes.data || []).map((p) => [p.id, p]));
  const bodMap = Object.fromEntries((bodsRes.data || []).map((b) => [b.id, b]));
  const ubicMap = Object.fromEntries((ubicsRes.data || []).map((u) => [u.id, u]));
